...
namespace ed
{
		class Donante: public DonanteInterfaz
	{
		...
		*/
			Donante(string nombre="", string apellidos="", string grupo="0", string factor="positivo", int donaciones=0)
			{
				nombre_=nombre;
...
/*!
		...
		\fn inline int getDonaciones()const{return donaciones_;}.
	...
	*/
		...
		\fn inline void setDonaciones(int donaciones).
		\brief Método que modifica las donaciones del donante.
		\return Ninguno.
		\param donaciones de tipo int.
	*/
			inline void setDonaciones(int donaciones){
			 if(donaciones>=0)
			 	donaciones_=donaciones;
			};
	/*!
	...
	*/
			void leerDonante()
			{
				...
				cin>>f;
				setFactor(f);
				cout<<"Introduzca el número de donaciones que ha realizado:"<<endl;
				cin>>d;
				setDonaciones(d);
			}
	/*!
	...
	*/
			void imprimirDonante()
			{
				...
				cout<<"Donaciones:"<<donaciones_<<endl;
				cout<<"-----------------------------"<<endl;
			}
	/*!
		\name Operador =.
	...
	*/
			...
			{
				if(getNombre()==d.getNombre() && getApellidos()== d.getApellidos() && getDonaciones()==d.getDonaciones())
				{
					return true;
				}
				else
			...
			bool modificar(Donante d)
			{
				...
				do
				{
	               ...
	               switch(op)
	               {
	                        ...
	                        break;
	                        case 5:
	                        cout<<"Introduce nuevas donaciones"<<endl;
	                        setDonaciones(dn);
	                        case 6:
...
/*!
	...
	*/
			 int ini()
            {
	            ...
	            cout<<"--------5.Modificar Donaciones--------------"<<endl;
	            cout<<"-------------6.Salir de modificar-----------"<<endl;
	            cout<<"--------------------------------------------"<<endl;
